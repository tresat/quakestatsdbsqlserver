...
GO
IF NOT EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[DataProcedures].[spGetGameResults]') AND type in (N'P', N'PC'))
BEGIN
...
-- =============================================
CREATE PROCEDURE [DataProcedures].[spGetGameResults] 
	-- Add the parameters for the stored procedure here
	@pProcSpecificParam0 int,
	@pSortCol int = 4, 
	@pSortAsc bit = 1,
	@pStartRow int = 0,
	@pLimitRows int = 10,
...
BEGIN
	...
	UPDATE #results SET FlagRecoveries = (
		...
		FROM CalculatedData.ClientToItem ci
			WHERE ci.fkGameID = @pProcSpecificParam0
	...
	WHERE clientID IN (
		...
		WHERE c.fkGameID = @pProcSpecificParam0
